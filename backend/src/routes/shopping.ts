...
import { shoppingItems } from '../data/hardcodedData'
...
router.put('/:id', (req, res) => {
  try {
    ...
    shoppingItems[itemIndex] = { ...shoppingItems[itemIndex], ...req.body }
...
router.get('/priority/:priority', (req, res) => {
  try {
    const priority = req.params.priority as 'high' | 'medium' | 'low'
    const items = shoppingItems.filter(item => item.priority === priority)
